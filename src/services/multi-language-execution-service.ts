    ...
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).__TAURI__) {
        try {
          ...
          const result = await invoke('parse_navlambda_code', { code });
          const executionResult = await invoke('compile_to_target', {
            code,
            target: 'python', // Execute via Python for now
          }) as { success: boolean; output?: string; compiled?: string; error?: string };
          ...
          if (executionResult.success) {
            ...
            if (executionResult.output) {
              outputs.push({
                ...
                execution_count: count,
                data: {
                  'text/plain': executionResult.output,
                },
              });
            }
            ...
            throw new Error(executionResult.error || 'NAVΛ execution failed');
          }
        } catch (tauriError) {
          // Fall through to simulation
        }
